...
```{r Load input data, echo=FALSE}
  ...
  cellCycle <- read.table(cellCycle_path, header=TRUE)
...
# Thresholds (filter counts)
  minCells = 5
  minCounts = 5

# Filter genes
  genes_filt = filter_genes(counts, minCounts, minCells)
    
    counts_filt <- counts[genes_filt, ]
    rpkm_filt <- rpkm[genes_filt, ]
    biomart_filt <- biomart %>% filter(GeneStableID %in% genes_filt)

    cellCycle_filt <- cellCycle %>% filter(GeneID %in% toupper(biomart_filt$GeneName))

### Add Gene annotations
  cellCycle_filt$mouse <- biomart_filt[match(cellCycle_filt$GeneID, toupper(biomart_filt$GeneName)), ]$GeneName
  cellCycle_filt$ensembl <- biomart_filt[match(cellCycle_filt$GeneID, toupper(biomart_filt$GeneName)), ]$GeneStableID
  colnames(cellCycle_filt) <- c('human', 'mouse', 'ensembl')
  
```


```{r Find most variable cell cycle genes using Seurat}
...
```{r Plot most variable cell cycle genes, fig.height = 4, fig.width = 4}
...
```{r PCA using most varible cell cycle genes, fig.height = 4, fig.width = 4}
...
```{r Plot PCA with cell cycle expression, fig.height = 4, fig.width = 16}
...
```{r Use princurve and PCA coordinates to align cells to cell cycle progression, fig.height = 4, fig.width = 8}
...
```{r Plot PC1 vs PC2 colored by cell cycle phase, fig.height = 4, fig.width = 4.5}
...
```{r Cell cycle genes colored by cell cycle phase, fig.height = 4, fig.width = 8}
...
```{r Anova for differential expression of lncRNAs}
...
```{r Plot anova stats vs fold changes, fig.height = 3, fig.width = 9}
...
```{r Expression of lncRNA candidates during cell cycle progression (rolling mean),  fig.height = 8, fig.width = 12}
